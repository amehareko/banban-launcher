...
**🙈 隐私子系统**：隐藏/查看/密码三件事，环环相扣但都不依赖输入法：

- **隐藏**：长按列表项 600ms → 二次确认 → 包名写入 `localStorage`。
  长按与点按**共用同一套防误触状态机**（scrollTop 快照 + 位移 slop + 时长 + 延迟执行），
  所以滚动列表时不会误弹隐藏确认。隐藏后 5 秒内可一键撤销。
- **查看**：长按右下角 ▦ → 内置数字键盘校验密码 → 进入「管理模式」，
  隐藏项半透明显示并带「已隐藏」角标，长按可恢复；关掉列表即自动退出管理模式。
- **密码**：4~8 位，自绘 0-9 键盘（**不用 `<input>`**——老设备唤起 IME 后字符卡在组合态，
  打不进去）。忘了也有自救：设置 → 隐私 → 长按「修改」3 秒可清除，隐藏列表不受影响。
- **两条防呆**：开关开着但没设密码时自动降级免密，不会把自己锁在外面；
  写入失败会提示「保存失败」而不是假装成功。
- **幽灵清理**：应用卸载后包名会赖在隐藏列表里。每次拿到真实应用列表时清理一次，
  但**只在列表不比历史最大值短时才清**，避免数据不全时误清掉还装着的隐藏应用。

**👵 老内核兼容**：页面全部 ES5 写法（无箭头函数 / let / 模板字符串），
主题切换靠 CSS 变量 + class 组合，老 Chromium 也能丝滑过渡。
出包前可跑 `node tools/es5-check.js` 体检——一处 ES6 语法就会让整页白屏且毫无报错。
...
110 项断言覆盖启动器双通道、天气位置编辑、一言类型选择、触屏 tap 判定、
